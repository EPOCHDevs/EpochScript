...
public:
  ...
  TransformData(const epoch_frame::DataFrame &bars) const override {
    // Setup LIBLINEAR logging to spdlog
    ...
    if (train_size < X.n_rows && pred_start < X.n_rows) {
      ...
      training_X = X.rows(0, train_size - 1);
      training_y = y.subvec(0, train_size - 1);

      // Predict on remaining rows (after gap)
      prediction_X = X.rows(pred_start, X.n_rows - 1);
...
private:
  ...
  epoch_frame::DataFrame GenerateOutputs(
    ...
    if constexpr (IsClassifier) {
      ...
      std::vector<double> prob_estimates(std::max(1, nr_class));
      // For nr_class=1, formula gives 0, but we need at least 1 element
      std::vector<double> dec_values(std::max(1, nr_class * (nr_class - 1) / 2));
      ...
      // Regressor outputs: single "result" output
      ...
      // SVR metadata defines output as "result" not "prediction"
      output_columns.push_back(GetOutputId("result"));
      output_arrays.push_back(epoch_frame::factory::array::make_array(predictions));
    }
