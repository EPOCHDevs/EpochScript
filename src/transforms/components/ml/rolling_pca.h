 ...
 * - n_components: Number of PCs to keep (0 = use variance_retained)
 * - variance_retained: Keep components until this variance (0.95 = 95%)
 */
class RollingPCATransform final
    : public RollingMLBaseUnsupervised<RollingPCATransform, PCAModel> {
public:
  using Base = RollingMLBaseUnsupervised<RollingPCATransform, PCAModel>;
  using OutputVectors = RollingPCAOutputs;

  explicit RollingPCATransform(const TransformConfiguration& cfg)
      : Base(cfg) {
    m_n_components = static_cast<size_t>(
        cfg.GetOptionValue("n_components",
                           MetaDataOptionDefinition{0.0}).GetInteger());

    m_variance_retained = cfg.GetOptionValue("variance_retained",
                                              MetaDataOptionDefinition{0.95}).GetDecimal();

    // Determine max components for output sizing
    // Will be clamped based on actual features during transform
    m_max_components = (m_n_components > 0) ? m_n_components : 10;
  }
  ...
  [[nodiscard]] PCAModel TrainModel(const arma::mat& X) const {
    ...
    // Apply PCA
    mlpack::PCA<> pca(false);  // No scaling - handled by preprocessing
    ...
    size_t n_comp = n_features;
    if (m_n_components > 0 && m_n_components < n_features) {
      n_comp = m_n_components;
  ...
  void Predict(const PCAModel& model,
    ...
    for (size_t i = 0; i < n_points; ++i) {
      ...
      for (size_t k = 0; k < model.n_components && k < m_max_components; ++k) {
        if (k < outputs.principal_components.size()) {
  ...
  [[nodiscard]] std::vector<std::string> GetOutputColumnNames() const {
    ...
    for (size_t k = 0; k < m_max_components; ++k) {
      names.push_back(GetOutputId("pc_" + std::to_string(k)));
    }
    names.push_back(GetOutputId("explained_variance_ratio"));
    return names;
  }
  ...
  void InitializeOutputVectors(OutputVectors& outputs, size_t n_rows) const {
    outputs.principal_components.resize(m_max_components);
    for (size_t k = 0; k < m_max_components; ++k) {
...
private:
  size_t m_n_components{0};
  double m_variance_retained{0.95};
