...
#include "transforms/components/utility/asset_ref.h"
...
TEST_CASE("EvaluateAssetRefClass with filter returns true (placeholder)", "[asset_ref][class]") {
  // Current implementation returns true for all filters (TODO: metadata lookup)
  REQUIRE(EvaluateAssetRefClass("AAPL", "Stocks") == true);
  REQUIRE(EvaluateAssetRefClass("BTC", "Crypto") == true);
  REQUIRE(EvaluateAssetRefClass("ES", "Futures") == true);
  REQUIRE(EvaluateAssetRefClass("EURUSD", "FX") == true);
  REQUIRE(EvaluateAssetRefClass("SPX", "Indices") == true);
}

// ============================================================================
// AssetRef transform tests
// ============================================================================

TEST_CASE("AssetRef transform throws on direct call", "[asset_ref][transform]") {
  const auto tf = EpochStratifyXConstants::instance().DAILY_FREQUENCY;

  auto cfg = run_op("asset_ref", "asset_ref_test",
      {},
      {{"ticker", MetaDataOptionDefinition{std::string("SPY")}}},
      tf);

  AssetRef transform{cfg};
  ...
  auto df = make_dataframe<double>(index, {{1.0, 2.0, 3.0}}, {"dummy"});

  // AssetRef::TransformData should throw
  REQUIRE_THROWS_WITH(transform.TransformData(df),
      Catch::Matchers::ContainsSubstring("should not be called directly"));
}

TEST_CASE("AssetRef transform construction with ticker option", "[asset_ref][transform]") {
  const auto tf = EpochStratifyXConstants::instance().DAILY_FREQUENCY;

  auto cfg = run_op("asset_ref", "asset_ref_ticker",
      {},
      {{"ticker", MetaDataOptionDefinition{std::string("AAPL")}}},
      tf);

  // Should construct without throwing
  REQUIRE_NOTHROW(AssetRef{cfg});
}

TEST_CASE("AssetRef transform construction with asset_class option", "[asset_ref][transform]") {
  const auto tf = EpochStratifyXConstants::instance().DAILY_FREQUENCY;

  auto cfg = run_op("asset_ref", "asset_ref_class",
      {},
...
TEST_CASE("AssetRef transform construction with both options", "[asset_ref][transform]") {
  const auto tf = EpochStratifyXConstants::instance().DAILY_FREQUENCY;

  auto cfg = run_op("asset_ref", "asset_ref_both",
      {},
      {{"ticker", MetaDataOptionDefinition{std::string("SPY")}},
       {"asset_class", MetaDataOptionDefinition{std::string("Stocks")}}},
      tf);
